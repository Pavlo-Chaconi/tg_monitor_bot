...
from commands import cmd_alerts, cmd_billing, cmd_help, cmd_report, cmd_restic
from config import load_config
from email_monitor import check_all_mailboxes
...
async def main():
    bot_cfg, prom_cfg, tn_cfg, sched_cfg, email_cfg = load_config()
    ...
    logger.info("Команды бота: /report /alerts /restic /billing /help")
    ...
    if email_cfg.mailboxes:
        scheduler.add_job(
            check_all_mailboxes,
            trigger=IntervalTrigger(hours=email_cfg.check_interval_hours),
            args=[email_cfg.mailboxes, bot, bot_cfg.chat_id],
            id="email_check",
            replace_existing=True,
        )
        logger.info(
            "Мониторинг почты: %d ящик(а), интервал %d ч",
            len(email_cfg.mailboxes),
            email_cfg.check_interval_hours,
        )
    scheduler.start()
    logger.info(
